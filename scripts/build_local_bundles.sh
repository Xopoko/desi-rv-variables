...
audit_artifact_dir="${DESI_RV_AUDIT_ARTIFACT_DIR:-$projects_root/desi-rv-audit-public/reports/program_night_artifacts}"
strict_candidates="${DESI_RV_STRICT_CANDIDATES:-$repo_root/artifacts/inputs/candidate_sources_strict.csv}"
strict_candidates_url="${DESI_RV_STRICT_CANDIDATES_URL:-https://github.com/Xopoko/desi-rv-variables/releases/download/v0.1.1/candidate_sources_strict.csv.gz}"
...
if [[ ! -f "$strict_candidates" ]]; then
  ...
  curl -L "$strict_candidates_url" -o "$gz_path"
